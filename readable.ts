...
} from "https://esm.sh/@mozilla/readability@0.4.1?no-check";
...
async function parseDOMFromURL(
  url: string,
  _proxy: string,
  _strictSSL: boolean,
  userAgent: string
  ...
  if (mimeType == "text/htm") mimeType = "text/html";
...
async function parseDOM(
  ...
  const baseURLString =
    document.getElementsByTagName("base")[0]?.getAttribute("href") ?? url;
  if (baseURLString) {
